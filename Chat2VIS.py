...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        st.write("Uploaded CSV Data:", data.head())  # Displaying the uploaded data
...
# Initializing variables to store context
user_questions = []
model_responses = []
# Function to handle user questions and update context
def handle_user_question(new_question, model_output):
    user_questions.append(new_question)
    model_responses.append(model_output)
    # Here, you could use user_questions and model_responses to maintain context
    # and use it when processing new questions and rendering visualizations
    render_d3_visualization(model_output, None)  # Assuming model_output includes visualization instructions
# Function to render D3.js visualizations based on model output and data
def render_d3_visualization(model_output, data):
    # Here we would interpret the model_output to choose a D3.js snippet and populate it with data
    # For demonstration, a simple condition is used to choose a visualization based on the model_output
    if model_output == "bar_chart":
        with open("/mnt/data/Chat2VIS_Streamlit-main/Chat2VIS_Streamlit-main/d3_visualizations/bar_chart.html", "r") as file:
            d3_html_content = file.read()
    else:
        with open("/mnt/data/Chat2VIS_Streamlit-main/Chat2VIS_Streamlit-main/d3_visualizations/line_chart.html", "r") as file:
            d3_html_content = file.read()
    st.components.v1.html(d3_html_content, width=500, height=500)
    # Embedding the D3.js visualization
    with open("/mnt/data/Chat2VIS_Streamlit-main/Chat2VIS_Streamlit-main/d3_visualizations/d3_bar_chart.html", "r") as file:
        d3_html_content = file.read()
    st.components.v1.html(d3_html_content, width=500, height=500)
